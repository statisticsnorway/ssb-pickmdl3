...
trend can be calculated with the filter based methods of `x13`. The
`pickmdl` procedure differs from the default `automodel` procedure by
restricting the model choiche to an ordered list of five parsimonious
models. The first model on the ordered list that fulfills three
pre-defined criteria is selcted.
...
once a year and the sARIMA model order should be kept unchanged in line
with a predefined refreshment policy. Let us say that model
identification is done in January each year, based on data until the
preceding December. Throughout the year, the model is kept the same in
accordance with the Outliers refreshment policy. Thus, there is a risk
of major revisions in January each year, as the selected sARIMA model
then may change.

![Figure 1: Automodel
approach](introduction_files/figure-html/unnamed-chunk-3-1.png)

Figure 1: Automodel approach

This is what happens in the seasonal adjustment of the norwegian retail
index for nace 47.6. In December 2024, the seasonal adjustment was based
on a sARIMA model selected almost a year ago. At the moment of
selection, the automodel procedure rejected the AIRLINE model and
selected a sARIMA model of order $`(0,0,1)(0,1,1)_s`$. Turning to
January 2025, the sARIMA model is identified anew. The AIRLINE model is
still rejected, but the selected sARIMA model has now changed to
$`(1,0,0)(0,1,1)_s`$. The resulting seasonally adjusted series are shown
in figure 1, where the seasonal adjusted time series of Decemeber 2024
(black) is compared with the seasonal adjusted series of January 2025
(red.)

![Figure2: Pickmdl
approach](introduction_files/figure-html/unnamed-chunk-4-1.png)

Figure2: Pickmdl approach
...
$`(2,1,0)(0,1,1)_s`$ is considered acceptable.
...
using the pickmdl procedure. We see that the model change that resulted
from the first approach introduces considerable revisions of the
adjusted data. With the pickmdl approach, revisions are much smaller.

![Figure 3:
Revisions](introduction_files/figure-html/unnamed-chunk-5-1.png)

Figure 3: Revisions

## Limitations of the pickmdl procedure

A limitation of the pickmdl procedure, however, is that an inadequate
sARIMA model may be chosen when none of the five models on the pickmdl
list passes the criteria. In this case, the procedure by default selects
the AIRLINE model, potentially conflicting with the ESS’s guidelines on
Seasonal adjustment. There may be other suitable sARIMA models that are
not considered. To address this issue, the pickmdl3 package provides the
