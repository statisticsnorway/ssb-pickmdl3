...
refreshment policy throughout the year. With the pickmdl package, it is
easy to implement this best practice in a production process. A date for
model selection can be set in the `x13_pickdml`function by the options
...
done each January, based on data until the preceding December, as often
is the case at Statistics Norway. We use the norwegian rti in nace 47.6
and the standard specification rsa5c, with outliers turned off to
...
prevented when we specify identification date for model selection.
...
passes the criteria when the additional data is taken into account. That
is, whether the model selected based on data until December 2025 still
fits the data in July 2026. We see that that is no longer the case,
which is an indication that model change should be expected when the
model is to be re-identified in January 2027.

## Outliers

In the example above, we didn’t consider outliers. The default option in
`x13_pickmdl` is that outliers are identified by the shortened series in
the cases when model selection is set to a past date. The outliers
identified by the shortened series are kept throughout the year, and
only new ouliers after the identification date are searched for
throughout the year.

In the example below, we manipulate the series to identify an outlier in
June 2026, that is after model selection is done based on data up to and
...
identfied in the model selection step. If we take a closer look at the
...
detection span for new outliers is only from January 2026:
...
setting the option `identify_outliers = FALSE`. Comparing outliers the
